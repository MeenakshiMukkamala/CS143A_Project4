...
PROCESS_STACK_MEMORY_NEEDED: str = "needed_stack_memory_MB"
PROCESS_HEAP_MEMORY_NEEDED: str = "needed_heap_memory_MB"
...
class Simulator:
    ...
    def __init__(self, emulation_description_path: Path, logfile_path: str, student_logs: bool):
        ...
        for process in emulation_json[PROCESSES]:
            ...
            stack_memory_needed_mb = 10
            heap_memory_needed_mb = 0

            if PROCESS_STACK_MEMORY_NEEDED in process:
                assert(type(process[PROCESS_STACK_MEMORY_NEEDED]) is int)
                stack_memory_needed_mb = process[PROCESS_STACK_MEMORY_NEEDED]

            if PROCESS_HEAP_MEMORY_NEEDED in process:
                assert(type(process[PROCESS_HEAP_MEMORY_NEEDED]) is int)
                heap_memory_needed_mb = process[PROCESS_HEAP_MEMORY_NEEDED]
            ...
            process = Process(process[ARRIVAL], process[TOTAL_CPU_TIME], 0, priority, priority_changes, \
                              semaphore_p_events, semaphore_v_events, mutex_lock_events, mutex_unlock_events, \
                                process_type, stack_memory_needed_mb * MB_TO_BYTES,
                                heap_memory_needed_mb * MB_TO_BYTES, memory_events)
    ...
    def check_for_arrival(self):
        while len(self.arrivals) > 0 and self.arrivals[len(self.arrivals) - 1].arrival == self.elapsed_time:
            ...
            self.log(f"{new_process.process_type} process {self.next_pid} arrived with priority {new_process.priority} requesting {new_process.stack_memory_needed / MB_TO_BYTES}MB of stack and {new_process.heap_memory_needed / MB_TO_BYTES}MB of heap")
            kernel_response = self.kernel.new_process_arrived(self.next_pid, new_process.priority, new_process.process_type, new_process.stack_memory_needed, new_process.heap_memory_needed)
